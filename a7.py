...
class BayesClassifier:
    ...
    def classify(self, text: str) -> str:
        ...
        for token in tokens:
            pos_word_count = self.pos_freqs.get(token, 0)
            pos_prob = (pos_word_count + 1) / (total_pos_words + len(self.pos_freqs))
            pos_score += math.log(pos_prob)

            neg_word_count = self.neg_freqs.get(token, 0)
            neg_prob = (neg_word_count + 1) / (total_neg_words + len(self.neg_freqs))
